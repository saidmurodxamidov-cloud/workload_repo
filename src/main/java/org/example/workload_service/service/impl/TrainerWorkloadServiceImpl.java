...
public class TrainerWorkloadServiceImpl implements TrainerWorkloadService {
    ...
    public void processWorkload(String idempotencyKey,TrainerWorkloadRequest request) {
        ...
        if(idempotencyKey != null &&
            repository.existsByIdempotencyKey(idempotencyKey)){
            log.info("duplicate key is identified key: {}", idempotencyKey);
            return;
        }
    ...
    public TrainerWorkloadResponse getSummary(String username){
        ...
        workloads.forEach(trainerWorkload -> {
            ...
            map.get(trainerWorkload.getYear()).put(MonthEnum.fromInt(trainerWorkload.getMonth()).getDisplayName(),trainerWorkload.getTotalDuration());
        });
